...
void main_window::create_socket_client(QString ip, quint16 port, int type)
{
    ...
    connect(socket_client, SIGNAL(nt_connect_error(QString)), this, SLOT(update_status_connect_error(QString)));
    connect(socket_client, SIGNAL(nt_join_error(QString)), this, SLOT(update_status_join_error(QString)));
    ...
    if(type == PROTOCOL_TYPE_MULTICAST){
        socket_client->udp_multicast_create();
    }
}
